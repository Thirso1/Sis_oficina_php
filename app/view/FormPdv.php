...
class FormPdv extends TPage
{
	...
	function __construct()
	{
		...
		// creates the form
		$this->form = new BootstrapFormBuilder('formPdv');
		...
		$id          = new TEntry('id');
		$date        = new TDate('date');
		$id_cliente = new TDBUniqueSearch('id_cliente', 'connect', 'Cliente', 'id', 'nome');
		$endereco         = new TEntry('endereco');
		$numero         = new TEntry('numero');
		$bairro         = new TEntry('bairro');
		$cidade         = new TEntry('cidade');

		$button = new TActionLink('', new TAction(['FormPdv', 'onEdit']), 'green', null, null, 'fa:plus-circle');
		$button->class = 'btn btn-default inline-button';
		$button->title = _t('New');
		$id_cliente->after($button);

		// detail fields
		$product_detail_unqid      = new THidden('product_detail_uniqid');
		$product_detail_id         = new THidden('product_detail_id');
		$product_detail_product_id = new TDBUniqueSearch('product_detail_product_id', 'connect', 'Produto', 'id', 'descricao');
		...
		$id_cliente->setSize('calc(100% - 30px)');
		$id_cliente->setMinLength(1);
		$date->setSize('100%');
		...
		$id_cliente->addValidation('Cliente', new TRequiredValidator);
		...
		$col_descr->setTransformer(function ($value) {
			return Produto::findInTransaction('connect', $value)->descricao;
		});
	...
	{
		$data = new stdClass;
		$data->id_cliente   = $param['id_cliente'];
	...
	public function onEdit($param)
	{
		...
		try {
			...
			if (isset($param['key'])) {
				...
				$itens_venda = Itens_venda::where('id_venda', '=', $object->id)->load();
